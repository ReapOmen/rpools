...
using efficient_pools4::PoolHeader;

template<typename T>
void test_pool_size() {
    LinkedPool4<T> lp;
    size_t expectedSize = (LinkedPool4<TestObject>::PAGE_SIZE -
                           sizeof(PoolHeader)) / sizeof(T);
    REQUIRE(lp.getPoolSize() == expectedSize);
}

TEST_CASE("Pool size of LinkedPool4 is correct for different objects",
          "[LinkedPool4]") {
...
void test_allocation_2() {
    ...
    REQUIRE(((size_t)objs[size - 2] & LinkedPool4<T>::POOL_MASK)
            != ((size_t)objs[0] & LinkedPool4<T>::POOL_MASK));
...
TEST_CASE("(De)allocation sequence produces correct result", "[LinkedPool4]") {
    SECTION("TestObject") {
...
TEST_CASE("A new pool allocates iff all the other pools are full", "[LinkedPool4]") {
    SECTION("TestObject") {

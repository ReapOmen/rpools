...
using efficient_pools2::PoolHeader;

template<typename T>
void test_pool_size() {
    LinkedPool2<T> lp;
    size_t expectedSize = (LinkedPool2<TestObject>::PAGE_SIZE -
                           sizeof(PoolHeader)) / sizeof(T);
    REQUIRE(lp.getPoolSize() == expectedSize);
}

TEST_CASE("Pool size of LinkedPool2 is correct for different objects",
          "[LinkedPool2]") {
...
void test_allocation_2() {
    ...
    REQUIRE(((size_t)objs[size - 2] & LinkedPool2<T>::POOL_MASK)
            != ((size_t)objs[0] & LinkedPool2<T>::POOL_MASK));
...
TEST_CASE("(De)allocation sequence produces correct result", "[LinkedPool2]") {
    SECTION("TestObject") {
...
TEST_CASE("A new pool allocates iff all the other pools are full", "[LinkedPool2]") {
    SECTION("TestObject") {
